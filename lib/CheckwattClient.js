...
/** EiB /ems/energyflow: GridNow is already in watts (unofficial; differs from BatteryNow/SolarNow in kW). */
function gridNowToWatts(energyData) {
  if (!energyData || typeof energyData.GridNow !== 'number' || !Number.isFinite(energyData.GridNow)) {
    return null;
  }
  return Math.round(energyData.GridNow);
}
...
class CheckwattClient {
  ...
  /** Momentan elnätseffekt (W). GridNow från API är redan watt — multiplicera inte med 1000. */
  get gridPowerW() {
    return gridNowToWatts(this.energyData);
  }
