...
/**
 * EiB GET /ems/energyflow: GridNow, BatteryNow, SolarNow are already in watts (float).
 * Do not multiply by 1000 — that was a mistaken kW assumption from older community docs.
 */
function instantPowerWatts(energyData, key) {
  if (!energyData || typeof energyData[key] !== 'number' || !Number.isFinite(energyData[key])) {
    return null;
  }
  return Math.round(energyData[key]);
}
...
class CheckwattClient {
  ...
  /** Momentan elnätseffekt (W). */
  get gridPowerW() {
    return instantPowerWatts(this.energyData, 'GridNow');
  }

  /** Momentan batterieffekt (W). */
  get batteryPowerW() {
    return instantPowerWatts(this.energyData, 'BatteryNow');
  }

  /** Momentan soleffekt (W). */
  get solarPowerW() {
    return instantPowerWatts(this.energyData, 'SolarNow');
  }
